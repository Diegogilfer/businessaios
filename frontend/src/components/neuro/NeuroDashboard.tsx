    ...
    <div className="animate-fade-in">
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 10, marginBottom: 24 }}>
          {[
            ...
            <div key={l as string} style={{ background: '#0d0d0d', borderTop: `2px solid ${c as string}`, border: `1px solid ${(c as string)}18`, padding: '14px 18px', borderRadius: 2 }}>
              ...
              <p style={{ fontSize: 16, fontWeight: 700, color: c as string, fontFamily: 'DM Mono, monospace', marginTop: 6 }}>{v as string}</p>
            </div>
          ))}
        </div>
      )}
      ...
      {result && (
        <div style={{ background: '#0d0d0d', border: '1px solid #c8f04a18', borderLeft: '3px solid #c8f04a', borderRadius: 2, padding: 20, animation: 'fadeIn .4s ease' }}>
          ...
          <pre style={{ fontSize: 11, color: '#666', fontFamily: 'DM Mono, monospace', whiteSpace: 'pre-wrap', wordBreak: 'break-all', maxHeight: 320, overflowY: 'auto', margin: 0 }}>
...
type Segment = { category: string; total_tasks: number; completion_rate: number; performance: string }
  ...
  return (
    <div style={{ background: '#0d0d0d', border: `1px solid ${color}18`, borderTop: `2px solid ${color}`, borderRadius: 2, padding: 16 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 10 }}>
        ...
        <span style={{ fontSize: 18, fontWeight: 700, color, fontFamily: 'DM Mono, monospace' }}>{pct}%</span>
      </div>
  ...
  return (
    <div className="animate-fade-in">
      ...
      <div style={{ marginBottom: 24, display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end' }}>
        <div>
          <h2 style={{ fontSize: 18, fontWeight: 300, color: '#eee' }}>NeuroIA — Fase 12</h2>
          <p style={{ fontSize: 11, color: '#2a2a2a', marginTop: 3 }}>Executive Copilot · Predicción · Segmentación · Análisis profundo</p>
        </div>
        ...
        <div>
          {bLoading ? (
            <div style={{ padding: 48, textAlign: 'center' }}>
              <span style={{ fontSize: 20, color: '#c8f04a', animation: 'spin 1.5s linear infinite', display: 'inline-block' }}>◌</span>
              <p style={{ fontSize: 11, color: '#333', marginTop: 12 }}>Generando briefing ejecutivo con IA...</p>
            </div>
          ) : briefData ? (
            <div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 10, marginBottom: 20 }}>
                {[
                  ['Completadas 7d',  briefData.metrics?.completed_7d,  '#c8f04a'],
                  ['Quality Score',   `${Math.round(Number(briefData.metrics?.avg_quality ?? 0) * 100)}%`, '#4af0c8'],
                  ['Opps Viables',    briefData.metrics?.viable_opps,   '#f0a44a'],
                  ['Grade del Sistema', briefData.metrics?.growth_grade, '#c44af0'],
                ].map(([l, v, c]) => (
                  <div key={l as string} style={{ background: '#0d0d0d', borderTop: `2px solid ${c as string}`, border: `1px solid ${(c as string)}18`, padding: '14px 18px', borderRadius: 2 }}>
                    <p style={{ fontSize: 9, color: '#333', textTransform: 'uppercase', letterSpacing: '0.1em' }}>{l as string}</p>
                    <p style={{ fontSize: 22, fontWeight: 700, color: c as string, fontFamily: 'DM Mono, monospace', marginTop: 6 }}>{v as string ?? '—'}</p>
                  </div>
                ))}
              </div>
              {/* Briefing IA */}
              <div style={{ background: '#0d0d0d', border: '1px solid #c8f04a18', borderLeft: '3px solid #c8f04a', borderRadius: 2, padding: 24 }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 14 }}>
                  <span style={{ fontSize: 10, color: '#c8f04a', textTransform: 'uppercase', letterSpacing: '0.1em' }}>⬡ Executive Copilot</span>
                  <span style={{ fontSize: 10, color: '#2a2a2a', fontFamily: 'monospace' }}>{new Date(briefData.generated_at).toLocaleString()}</span>
                </div>
                <p style={{ fontSize: 13, color: '#bbb', lineHeight: 1.8, whiteSpace: 'pre-wrap' }}>{briefData.briefing}</p>
              </div>
            </div>
          ) : (
            <p style={{ color: '#333', fontSize: 12 }}>Error generando briefing — verifica que el backend está corriendo</p>
          )}
        </div>
      )}
      ...
      {tab === 'alerts' && (
        <div>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
            <p style={{ fontSize: 11, color: '#333' }}>Monitoreo proactivo — el sistema detecta situaciones sin que preguntes</p>
            <span style={{ fontSize: 10, fontFamily: 'monospace', color: '#2a2a2a' }}>{alertList.length} alertas activas</span>
          </div>
          {aLoading ? (
            <p style={{ color: '#333', fontSize: 12 }}>Escaneando sistema...</p>
          ) : alertList.length === 0 ? (
            <div style={{ background: '#0d0d0d', border: '1px solid #c8f04a18', borderRadius: 2, padding: 32, textAlign: 'center' }}>
              <p style={{ fontSize: 20, color: '#c8f04a', marginBottom: 8 }}>✓</p>
              <p style={{ fontSize: 12, color: '#555' }}>Sin alertas críticas — sistema en buen estado</p>
            </div>
          ) : (
      ...
      {tab === 'predictions' && (
        <div>
          <p style={{ fontSize: 11, color: '#333', marginBottom: 16 }}>Calidad predicha basada en historial real · media móvil ponderada</p>
          {pLoading ? (
            <p style={{ color: '#333', fontSize: 12 }}>Calculando predicciones...</p>
          ) : (
      ...
      {tab === 'segments' && (
        <div>
          {sLoading ? (
            <p style={{ color: '#333', fontSize: 12 }}>Analizando segmentos...</p>
          ) : segData ? (
            <div>
              {/* Insights */}
              {segData.insights?.length > 0 && (
                <div style={{ background: '#0d0d0d', border: '1px solid #4af0c818', borderLeft: '3px solid #4af0c8', borderRadius: 2, padding: 16, marginBottom: 20 }}>
                  <p style={{ fontSize: 10, color: '#4af0c8', textTransform: 'uppercase', letterSpacing: '0.1em', marginBottom: 10 }}>◈ Insights automáticos</p>
                  {segData.insights.map((ins, i) => (
                    <p key={i} style={{ fontSize: 12, color: '#888', marginBottom: 4, display: 'flex', gap: 6 }}>
                      <span style={{ color: '#4af0c8', flexShrink: 0 }}>→</span> {ins}
                    </p>
                  ))}
                </div>
              )}
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 10 }}>
                {(segData.segments ?? []).map((s, i) => {
                  ...
                  return (
                    <div key={s.category} style={{ background: '#0d0d0d', border: `1px solid ${c}18`, borderTop: `2px solid ${c}`, borderRadius: 2, padding: 16 }}>
                      ...
                      <p style={{ fontSize: 20, fontWeight: 700, color: c, fontFamily: 'DM Mono, monospace', marginBottom: 6 }}>{s.completion_rate}%</p>
                      <p style={{ fontSize: 10, color: '#444' }}>{s.total_tasks} tareas · {s.failure_rate}% fallos</p>
                    </div>
                  )
                })}
              </div>
      ...
      {tab === 'analyze' && (
        <div style={{ maxWidth: 700 }}>
          <p style={{ fontSize: 11, color: '#333', marginBottom: 18 }}>Análisis estratégico profundo generado por IA con tus datos reales</p>
          <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginBottom: 16 }}>
            {['general','growth','cost','quality','arbitrage'].map(area => (
              <button key={area} onClick={() => setAnalyzeArea(area)} style={{
                background: analyzeArea === area ? '#141414' : 'transparent',
                border: analyzeArea === area ? '1px solid #c8f04a44' : '1px solid #1e1e1e',
                color: analyzeArea === area ? '#c8f04a' : '#555',
                padding: '6px 14px', borderRadius: 2, fontSize: 10,
                cursor: 'pointer', textTransform: 'uppercase', letterSpacing: '0.06em', fontFamily: 'inherit',
              }}>{area}</button>
            ))}
          </div>
          <button onClick={runAnalysis} disabled={analyzing} style={{
            background: analyzing ? '#141414' : '#c8f04a',
            color: analyzing ? '#333' : '#000', border: 'none',
            padding: '11px 24px', borderRadius: 2, fontSize: 12,
          ...
          {analysisData && (
            <div style={{ background: '#0d0d0d', border: '1px solid #c8f04a18', borderLeft: '3px solid #c8f04a', borderRadius: 2, padding: 24, animation: 'fadeIn .4s ease' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 14 }}>
                <span style={{ fontSize: 10, color: '#c8f04a', textTransform: 'uppercase', letterSpacing: '0.1em' }}>⬡ Análisis: {analysisData.focus_area as string}</span>
                <span style={{ fontSize: 10, color: '#2a2a2a', fontFamily: 'monospace' }}>
                  {new Date(analysisData.generated_at as string).toLocaleString()}
                </span>
              </div>
              <p style={{ fontSize: 13, color: '#bbb', lineHeight: 1.8, whiteSpace: 'pre-wrap' }}>
